...
from .models import CUser, StaffNotification, BlacklistedToken
...
def send_email(user: CUser, context, subject, template):
...
class UpdateCurrentUserSerializer(BaseUserSerializer):
    ...
    def get_is_superuser(self, obj: CUser):
...
class UserSerializer(BaseUserSerializer):
    ...
    def update(self, instance, validated_data):
        email_field = get_user_email_field_name(CUser)
...
class UserCreateSerializer(BaseUserCreateSerializer):
    ...
    def validate_username(self, value):
        if CUser.objects.filter(username__iexact=value).exists():
            raise serializers.ValidationError("Username already exists.")

        if CUser.objects.filter(email__iexact=value).exists():
...
class ResendStaffInvitationSerializer(serializers.Serializer):
    ...
    def validate_email(self, value):
        try:
            user = CUser.objects.get(email=value)
            if user.username:
                raise serializers.ValidationError(
...
class CustomObtainPairSerializer(TokenObtainPairSerializer):
    def validate(self, attrs):
        ...
        user = CUser.objects.filter(**{self.username_field: username}).first()
...
class CustomSendEmailResetSerializer(SendEmailResetSerializer):
    def validate(self, attrs):
        email = attrs.get('email')
        user = CUser.objects.filter(email__iexact=email).first()
        if not user.is_active:
            raise serializers.ValidationError(
